...
const CURRICULUM_STYLES = [
  ...
  {id:"tgtb",       label:"TGTB",              icon:"📖", desc:"The Good and the Beautiful"},
  {id:"abeka",      label:"Abeka",             icon:"📚", desc:"Traditional Christian textbooks"},
  {id:"cc",         label:"Classical Conv.",   icon:"🔁", desc:"Memory work & weekly co-op"},
  ...
  {id:"eclectic",   label:"Eclectic",          icon:"🎨", desc:"Mix of what works best"},
  {id:"unschooling",label:"Unschooling",       icon:"🌱", desc:"Child-led, interest-based"},
  {id:"ai",         label:"AI-Built",          icon:"✨", desc:"Let Root & Bloom build it"},
];
...
/* ═══════════════════════════ ROOT ═══════════════════════════ */
export default function RootAndBloom() {
  ...
  return (
    <div style={{
      ...
      {view==="parent" && family && (<>
        ...
        {/* Modals */}
        {checkin      && <CheckinModal   pal={pal} family={family} onClose={()=>setCheckin(false)} onSave={addPortfolioEntry} />}
...
function StudentPortal({ child, family, pal, isCoop, allEntries, onAddEntry, onCoopLog, onBack }) {
  ...
  const SCHEDULE = [
    {t:"8:00",s:"Morning Basket",i:"🌅",dur:30},
    {t:"8:30",s:"Math",i:"📐",dur:60},
    {t:"9:30",s:"Language Arts",i:"📝",dur:60},
    {t:"11:00",s:"History Together",i:"🏛️",dur:45},
    {t:"1:30",s:"Science",i:"🔬",dur:60},
    {t:"2:30",s:"Art",i:"🎨",dur:45},
  ];
...
function OnboardingFlow({ pal, onComplete }) {
  ...
  const STEPS = ["Welcome","Your Children","Curriculum","Schedule","Goals","Life & Extras"];
  ...
  return (
    <div style={{minHeight:"100vh",background:pal.sand,display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,overflowY:"auto"}}>
        ...
        {step===2 && <OnbCurriculum pal={pal} data={data} upd={upd} onNext={()=>setStep(3)} />}
        {step===3 && <OnbSchedule   pal={pal} data={data} upd={upd} onNext={()=>setStep(4)} />}
        {step===4 && <OnbGoals      pal={pal} data={data} upd={upd} onNext={()=>setStep(5)} />}
        {step===5 && <OnbExtras     pal={pal} data={data} upd={upd} onNext={()=>onComplete(data)} />}
      </div>
    </div>
  );
}
...
/* ─── STEP 2: Curriculum ─── */
function OnbCurriculum({ pal, data, upd, onNext }) {
  return (
    <div style={{padding:"1.4rem 1.2rem",animation:"fadeUp 0.25s ease"}}>
      <StepHdr pal={pal} icon="📚" title="Curriculum Style" sub="How does your family like to learn? Pick what fits best." />
      <div style={{display:"flex",flexDirection:"column",gap:"0.5rem",marginBottom:"1.2rem"}}>
        {CURRICULUM_STYLES.map(s=>{
          const sel=data.curriculumStyle===s.id;
          return (
            <button key={s.id} onClick={()=>upd("curriculumStyle",s.id)} style={{padding:"0.85rem 1rem",border:`2px solid ${sel?pal.primary:pal.stone+"50"}`,borderRadius:"14px",background:sel?pal.pale:"transparent",cursor:"pointer",textAlign:"left",display:"flex",gap:"0.75rem",alignItems:"center",transition:"all 0.14s"}}>
              <span style={{fontSize:"1.4rem",flexShrink:0}}>{s.icon}</span>
              <div style={{flex:1}}>
                <div style={{fontWeight:"700",color:sel?pal.primary:pal.ink,fontSize:"0.86rem"}}>{s.label}</div>
                <div style={{fontSize:"0.72rem",color:pal.slate,marginTop:"1px"}}>{s.desc}</div>
              </div>
              <div style={{width:"19px",height:"19px",borderRadius:"50%",border:`2px solid ${sel?pal.primary:pal.stone}`,background:sel?pal.primary:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                {sel&&<span style={{color:pal.onDark,fontSize:"0.6rem",fontWeight:"900"}}>✓</span>}
              </div>
            </button>
          );
        })}
  ...
  return (
    <div style={{padding:"1.4rem 1.2rem",animation:"fadeUp 0.25s ease"}}>
      ...
      <div style={{background:pal.heroGrad,borderRadius:"18px",padding:"1.2rem 1.3rem",marginBottom:"1.1rem",textAlign:"center",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{fontSize:"0.78rem",color:"rgba(255,255,255,0.65)",lineHeight:1.65,position:"relative"}}>Root & Bloom is ready for your family. Let's grow something beautiful together.</div>
      </div>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"1.2rem 1.1rem"}}>
        ...
        {tab==="school" && (
          <div style={{animation:"fadeUp 0.18s ease"}}>
            ...
            <SCard pal={pal} title="Curriculum Style">
              <div style={{display:"flex",flexDirection:"column",gap:"0.38rem"}}>
                {CURRICULUM_STYLES.map(s=>{
                  const sel=local.curriculumStyle===s.id;
                  return (
                    <button key={s.id} onClick={()=>updL("curriculumStyle",s.id)} style={{display:"flex",gap:"0.65rem",alignItems:"center",padding:"0.65rem 0.85rem",border:`2px solid ${sel?pal.primary:pal.stone+"50"}`,borderRadius:"11px",background:sel?pal.pale:"transparent",cursor:"pointer",textAlign:"left",transition:"all 0.13s"}}>
                      <span style={{fontSize:"1.2rem"}}>{s.icon}</span>
                      <span style={{fontWeight:sel?"700":"400",color:sel?pal.primary:pal.inkM,fontSize:"0.83rem"}}>{s.label}</span>
                    </button>
                  );
                })}
              </div>
...
function HomeScreen({ pal, family, child, setChild, onCheckin, onTheme }) {
  const blocks = [
    {time:"8:00",subject:"Morning Basket",icon:"🌅",child:"all",dur:30},
    {time:"8:30",subject:"Math",icon:"📐",child:family.children[0]?.id,dur:60},
    {time:"9:30",subject:"Language Arts",icon:"📝",child:family.children[0]?.id,dur:60},
    {time:"11:00",subject:"History Together",icon:"🏛️",child:"all",dur:45},
    {time:"1:30",subject:"Science",icon:"🔬",child:family.children[0]?.id,dur:60},
    {time:"2:30",subject:"Art",icon:"🎨",child:"all",dur:45},
  ];
  const visible = blocks.filter(b=>child==="all"||b.child===child||b.child==="all");
  const pct = 71;
      ...
      <div style={{padding:"1rem"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:"0.42rem",marginBottom:"1rem"}}>
          {visible.slice(0,5).map((b,i)=>{
            const ch=family.children.find(c=>c.id===b.child);
            const tint=b.child==="all"?pal.primary:pal.mid;
            return (
              <div key={i} style={{background:pal.linen,borderRadius:"13px",padding:"0.65rem 0.85rem",display:"flex",gap:"0.75rem",alignItems:"center",border:`1.5px solid ${pal.stone}45`}}>
                <div style={{width:"36px",height:"36px",borderRadius:"9px",background:`${tint}18`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.1rem",flexShrink:0}}>{b.icon}</div>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontWeight:"700",color:pal.ink,fontSize:"0.84rem",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{b.subject}</div>
                  <div style={{fontSize:"0.7rem",color:pal.slate}}>{h12(b.time)} · {b.dur}min</div>
                </div>
                <div style={{fontSize:"0.68rem",fontWeight:"700",color:pal.onDark,background:tint,borderRadius:"50px",padding:"0.1rem 0.55rem",flexShrink:0}}>{b.child==="all"?"All":ch?.name}</div>
              </div>
            );
          })}
        </div>
...
function ScheduleScreen({pal,family,child,setChild,onMarkComplete}){
  ...
  const BLOCKS=[{t:"8:00",s:"Morning Basket",i:"🌅",c:"all",dur:30},{t:"8:30",s:"Math",i:"📐",c:"c1",dur:60},{t:"9:30",s:"Language Arts",i:"📝",c:"c1",dur:60},{t:"11:00",s:"History Together",i:"🏛️",c:"all",dur:45},{t:"1:30",s:"Science Lab",i:"🔬",c:"c1",dur:60},{t:"2:30",s:"Art",i:"🎨",c:"all",dur:45}];

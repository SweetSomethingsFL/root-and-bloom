...
//
//  REVIEW LOG:
//  2026-04-29 — Added noi field to all 50 states: type, filedTo, frequency,
//              deadline, requiredFields, notarized, tip/noNoticeNote
//              Sources: state DOE sites, HSLDA, CRHE, florida-homeschooling.org,
//              homeschoolpennsylvania.org, NYC DOE, PHEA
...
function getStateInfo(state) {
  if(!state) return STATE_COMPLIANCE["Florida"] || {};
  // Try exact match first
  if(STATE_COMPLIANCE[state]) return STATE_COMPLIANCE[state];
  ...
  const full = ABBR[state.toUpperCase()];
  if(full && STATE_COMPLIANCE[full]) return STATE_COMPLIANCE[full];
  return {

      ...
      {isCoop ? (
        <div style={{background:"#fff8e6",borderRadius:"16px",padding:"0.9rem 1rem",border:"1.5px solid #f5c84250"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:"0.25rem"}}>
            {activeSubjs.map(s=>{
              const coopSubjs = data.coopSubjects||[];
              const sel = coopSubjs.includes(s.id);
              return (
                <button key={s.id} onClick={()=>{
                  const cur = data.coopSubjects||[];
                  upd("coopSubjects", sel?cur.filter(x=>x!==s.id):[...cur,s.id]);
                }} style={{display:"flex",gap:"0.5rem",alignItems:"center",padding:"0.4rem 0.55rem",border:`1.5px solid ${sel?"#b07800":"#f5c84280"}`,borderRadius:"8px",background:sel?"#fff3cc":"#fff8e6",cursor:"pointer",textAlign:"left",transition:"all 0.12s"}}>
          ...
          {(data.coopSubjects||[]).length===0&&<div style={{fontSize:"0.68rem",color:"#b07800",marginTop:"0.5rem",fontStyle:"italic",textAlign:"center"}}>{"Tap subjects above, or leave empty if you don't do regular subjects at co-op."}</div>}
        </div>
      ) : (
